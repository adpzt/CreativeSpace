...
**Fait 03/07 - emoji recherchable + récap Revenus :**
- **Emoji (post-it + tâches)** : le sélecteur macOS (🌐/Fn) n'insérait pas de façon fiable → on ABANDONNE la saisie libre. Nouveau `EmojiPicker` = **grande liste par catégories + barre de recherche FR** (insensible aux accents, `lib/emoji-data.ts` avec mots-clés français, `normalizeSearch`). Choix par clic = 100% fiable, plus aucune dépendance à l'OS. `NOTE_EMOJIS` (lib/notes) n'est plus utilisé.
- **Bank > Revenus** : au clic sur un revenu, on ouvre désormais un **récap en lecture** (façon fiche projet : titre, statut, type de mission, client, projet + date de fin, devis, net, date d'encaissement, notes) avec un bouton **Modifier** (crayon) qui bascule vers le formulaire. `RevenuForm` a un `mode` view/edit ; "Annuler" en édition revient au récap.
