...
3. **Emoji perso non applicable** ("+"/titre). CAUSES : (a) le "+" était un champ texte minuscule peu découvrable → remplacé par une grille d'emojis élargie (48) + un champ "Autre" clairement labellisé (taper/coller) ; (b) RACE : `updateNote` n'était pas attendu et `router.refresh()` à la fermeture pouvait réécraser l'emoji tapé en dernier avec les données serveur périmées → `closeEditing` attend désormais les écritures en cours (pendingSaves) avant refresh. **À reconfirmer sur le Brave d'Adrien.**
