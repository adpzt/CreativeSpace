...
**Bank > Revenus : n° de devis dans la fiche d'un revenu (02/08)** — en cliquant un revenu, le récap affiche maintenant `Devis n° …` (+ `Facture n° …` si renseigné) juste sous la ligne « Projet ». La source est le PROJET lié (`projects.devis_number` / `invoice_number`, déjà au schéma, saisis dans la fiche projet) : le revenu lui-même n'a pas de n° de devis, seulement `payments.invoice_ref` (réf d'encaissement, affichée en bas). Si un revenu est lié à un projet **freelance** sans n° de devis, on affiche « Devis n° non renseigné » (fiche muette = on croit à un bug). Vérifié au navigateur sur les 5 revenus réels d'Adrien (4 avec devis Malt/direct, 1 sans).
...
**PHASE 3 (Finance) terminée (à valider)** = page unique scindée en 2 grandes sections (SectionBanner) : **FREELANCE** (Dashboard, Revenus, Dépenses, Diagrammes, URSSAF, Seuils micro-BNC/TVA) et **SALAIRE & IMPÔT** (Salaires + Impôt sur le revenu estimé). Dissociation voulue par Adrien (le freelance ≠ le salaire/impôt). Le Dashboard est un recap par MOIS (sélecteur + flèches) avec bascule Mois/Année ; "en attente/dû" = indicateur global (jamais rattaché à un mois car compté à l'encaissement). Note d'encaissement affichée (date de réception, pas de facture). Salaires : année en select (2024..N+1, pour archiver stages 2025), liste groupée par année. PrevisionnelSection supprimé -> SeuilsSection (freelance) + ImpotSection (impôt + CA avant impôt annuel/mensuel). L'ancien objectif de CA éditable a disparu.
...
- **RÈGLE SALAIRE/IMPÔT (importante)** : sur un bulletin il y a 2 montants à NE PAS confondre. (1) "Net à payer / net payé" = argent versé sur le compte = trésorerie perso, N'ENTRE JAMAIS dans le calcul fiscal (champ `net_salary`). (2) "Net imposable" = base fiscale réelle du mois (champ `net_taxable`), à SAISIR MANUELLEMENT à chaque bulletin (ligne "Net imposable", ou cumul "depuis janvier"). Le net imposable n'est PAS déductible du net à payer par un % fixe (écart variable). Base fiscale = cumul annuel des `net_taxable` saisis. `apprentiTaxableSalary` prend ce cumul (PAS le brut, corrige l'ancien calcul). part_imposable_salaire = max(0, net_imposable_cumulé − SMIC_ANNUEL). SalaireForm : net imposable mis en avant (encadré bleu), net à payer marqué "trésorerie", brut optionnel.
- **WORK calendrier** : les livrables non cochés des projets CLÔTURÉS sont désormais proposés au calendrier (suggestionsFor exclut seulement les projets annulés), pour réajuster le planning même passé.
- **WORK livrable** : régler la progression à 100% coche automatiquement le livrable (et en dessous le décoche) - règle dans updateDeliverable (serveur) + sync local dans ProjectOverlayBody. Le toggle manuel de la case reste indépendant.
- **RÈGLE IMPÉRATIVE (bug corrigé)** : ne JAMAIS comptabiliser quelque chose qui n'est pas validé. Le Dashboard "Dépenses" et le bénéfice net ne comptent QUE les dépenses validées (table `expenses`), jamais les `mission_expenses` jsonb d'un projet non encore validées (sinon comptage avant validation + double comptage après). Idem revenus : seul `status="paid"` compte dans le CA. Les **dépenses n'entrent JAMAIS dans l'URSSAF ni dans l'impôt** (micro : URSSAF sur le CA, impôt via abattement forfaitaire 34%) — Adrien n'a pas de compte pro, il dépense sur son compte courant. Les dépenses ne servent que d'indicateur (carte Dépenses + bénéfice net).
- **Décision URSSAF/IMPÔT — BASE = FACTURÉ (BRUT), corrigé le 01/07** : la base de l'URSSAF ET de l'impôt = le montant **facturé** = `gross_amount` (prix du devis, ex. PACO 898€), PAS le net encaissé après commission (`net_amount` 790€). Only les paiements `status=paid` du mois comptent (encaissement), hors salaire, sans TVA. Fallback `gross_amount ?? net_amount`. Implémenté dans UrssafSection (encaisseOf + recap), DashboardSection (grossIn pour l'URSSAF ; le CA/bénéfice restent en net = argent réellement perçu), ImpotSection (CA fiscal = brut, abattement 34%), Home (alerte URSSAF). NB : `net_amount` reste la vue "argent réellement gagné" (CA card, bénéfice, diagrammes) ; le brut sert au fiscal.
- **Retours session 2 (suite) — réagencement Prévisionnel/URSSAF** : (1) L'ancien "Objectif de CA" éditable est SUPPRIMÉ (plus de profile ca_goal). Remplacé par une progression NON modifiable "CA avant impôt" = CA freelance à ne pas dépasser pour rester non imposable, annuel ET mensuel (annuel/12). (2) Le bloc "Repères fiscaux" est SUPPRIMÉ (répétitif) : taux URSSAF (actuel + après ACRE) affiché sous le titre URSSAF ; abattement 34% + tranche marginale déplacés dans la section Impôt ; micro-BNC/TVA restent dans l'onglet Seuils. (3) Section Impôt mise en avant (bordure bleue, header bleu, montant 4xl). (4) URSSAF : 3 vues (Mois = carrousel avec flèches, mois précédent grisé à gauche + 2 suivants à droite, mois courant encadré bleu ; 12 derniers mois = grille ; Depuis le début = recap total CA/URSSAF/mois déclarés). Plus de navigation par année (URSSAF est mensuel). getFinanceSettings/setFinanceSetting restent dans actions.ts mais ne sont plus utilisés.
- **CHIFFRES FISCAUX OFFICIELS (corrigés le 30/06, source Claude chat d'Adrien) — tous centralisés dans lib/finance.ts, datés, à actualiser chaque année** : URSSAF cotisations BNC = **25,6%** du CA. ACRE = **-50%** -> pendant l'ACRE **12,8%** (25,6%/2), après **25,6%**. (CORRIGÉ le 02/07 : le relevé RÉEL d'Adrien montre **117€ payés sur 898€ facturés** pendant l'ACRE = **13,03%**, soit un taux plein de **26,06%** (13,03% x 2). `URSSAF_COTISATION_BNC` passé à **0,2606**. Le libellé du taux dans UrssafSection est désormais calculé depuis la constante, plus de "12,8%/25,6%" en dur.) Fin ACRE = dernier jour du 3e trimestre civil suivant le début (juin 2026 -> **31/03/2027**), constante `ACRE_END={year:2027,month:3}`. Abattement micro-BNC **34%** (bénéfice imposable = CA x 66%). Barème IR **revenus 2026** : 0% < 11 600 ; 11% 11 601-29 579 ; 30% 29 580-84 577 ; 41% 84 578-181 917 ; 45% au-delà. SMIC_ANNUEL = 21 273 (exonération apprenti sur le BRUT). PAS de versement libératoire (déclaration annuelle). Helpers : `urssafRate`, `isAcre`, `estimateIncomeTax`, `currentBracket`, `apprentiTaxableSalary`. PrevisionnelSection affiche aussi la tranche marginale + la marge de CA avant la tranche suivante.
- **Réponses d'Adrien (confirmées)** : (a) URSSAF = **net encaissé** validé ✅. (b) **Poppins = alternance actuelle, The Source = prochaine alternance, les deux en APPRENTISSAGE** → salaire exonéré d'impôt jusqu'au SMIC annuel. Implémenté : `apprentiTaxableSalary()` dans lib/finance.ts, `SMIC_ANNUEL = 21 273` (à actualiser/an), part imposable = max(0, salaire annuel − SMIC). page.tsx calcule salaryTaxable post-exonération (net_taxable sinon net_salary). Son salaire ~1480/mois < SMIC → salaire imposable = 0. (c) **Type de mission sur le revenu** : ajouté en select OPTIONNEL dans RevenuForm (défaut "Selon le projet lié"), même vocabulaire que MISSION_TYPES. Migration 008 = `payments.mission_type` enum -> text (À EXÉCUTER). Diagrammes : priorité au type saisi sur le revenu, sinon dérivé du projet lié.
...
**PHASE 4 (Freelance) TERMINÉE (à valider)** : hub /freelance (5 cartes) + les 5 sous-sections. Tout le contenu statique est dans lib/freelance.ts (SCRIPTS, TUNNEL_STEPS, RED_FLAGS, BRIEF_QUESTIONS, DEVIS/CGP, PRODUCTION, PROSPECT_*). Vues client dans components/freelance/ (CopyButton réutilisable). 4.1 Communication (tunnel 7 étapes, 13 scripts copiables, red flags). 4.2 Brief (questions par type, copier ; bouton Google Form prêt, URL à fournir = BRIEF_FORM_URL). 4.3 Devis & Facture (checklists, pénalités copiables, 11 articles CGP + "tout copier", liens). 4.4 Production (structure dossiers copiable, nommage, règles, checklist livraison, arnaqueurs). 4.5 Prospection (liens rapides + board prospects CRUD : table `prospects` déjà au schéma, actions dans app/(main)/freelance/actions.ts, pas de migration). À FOURNIR par Adrien : URL Google Form brief, lien Canva devis.
...
**APPLICATION DU DESIGN SYSTEM (en cours, 01/07) — handoff dans design_handoff_creative_space/ (README = recettes Tailwind, gitignoré)** : progression par points de retour :
- ✅ P.R.1 TOKENS : fusionnés dans tailwind.config.ts (couleurs ink #191919/ink-soft/amber, ombres card/lift/float/sheen/chip, easings ease-ios/ease-spring, duration-180, animations rise/pop/sheet/fade-in/drift) + globals.css (prefers-reduced-motion, body ink #191919).
- ✅ P.R.2 NOTES : cartes post-it/glass (dégradé priorité en filigrane, shadow custom+sheen, case ronde neutre→pop, footer dot+priorité+échéance, animate-rise échelonné, terminée pâle, poubelle au survol). PRIORITIES enrichi (color/labelColor/grad) dans lib/notes.ts. Éditeur = page Notion (lecture: crayon icône, titre 34px, propriétés en lignes, contenu 15.5px ; édition: badge "en cours" + titre outline focus). Overlay = feuille (scrim flouté bg-black/[0.32] + animate-sheet + shadow-float + poignée).
- ✅ P.R.3 SEMAINIER : board bg-[#FCFCFD] rounded-[20px], en-têtes jours + pastille "Aujourd'hui" encre (bg-ink), boîtes catégorie = cartes blanches shadow-card avec TEXTE COLORÉ (pas de dot — choix Adrien, dévie du handoff qui mettait un dot + texte neutre), cellules = tuiles bg-[#F1F1F4] rounded-xl, chips = cartes blanches shadow-chip + hover lift, DragOverlay shadow-float.
- ✅ P.R.4 COMPOSANTS + BARRES + STATS : Button (primaire encre hover:-y/active:scale, secondaire, ghost, danger — components/ui/Button.tsx), ProgressBar (piste black/[0.07], fill transition width 300ms), Sidebar (actif bg-ink text-white, inactif ink-soft), Header (titre 17px bold), BottomNav (actif text-active), cartes stats Finance (DashboardSection Metric : shadow-card + hover lift + animate-rise, icône tuile rounded-[9px], valeur 3xl bold).
- ✅ P.R.5 PAGE-NOTION LIVRABLES/CALENDRIER (le GROS, 01/07) : NotePanel (components/ui/NotePanel.tsx) refondu pour s'ouvrir en LECTURE façon Notion (grand titre 34px, propriétés meta, séparateur hairline, contenu HTML aéré "Aucun détail." si vide) avec crayon en haut à gauche pour basculer en ÉDITION (badge/bouton "Terminé" bleu, titre en focus outline si onTitleSave, **RichText** pour le contenu à la place du vieux textarea). Contenu stocké en HTML (comme les Notes) ; rendu view via dangerouslySetInnerHTML + whitespace-pre-wrap (compat legacy plain-text). Autosave contenu + titre conservés. Scrim passé en bg-black/[0.32] backdrop-blur-[3px], panneau en shadow-float/ease-ios. Vaut pour les 3 appelants (CalendarSection blocs, ProjectOverlayBody livrables, ProjectCreateForm). meta/footer restent des ReactNode inline-éditables passés par les parents (inchangés). Aligné sur components/notes/NoteEditor.tsx.
- ✅ P.R.6 FINITIONS DESIGN (01/07) : (a) mini-graphe Recharts AreaChart sur la carte "CA encaissé" du Dashboard (composants/finance/DashboardSection Metric prop `spark` = CA net des 6 derniers mois ; dégradé vertical #2563EB 0.28→0, stroke 2.5px, h-12, pas d'axes, masqué si tout à 0). (b) badges de statut au format design 2.7 : StatusBadge en rounded-full px-3 py-1.5 text-[12.5px] font-semibold + dot h-2 ; **"en attente" passé en ORANGE** (signal `pending` : PROJECT_STATUS waiting_* + PAYMENT_STATUS pending en bg-orange-50 text-pending, dot bg-pending — remplace l'ancien gris ; à valider par Adrien) ; pills inline (RevenusSection, ProspectsBoard) alignées 12.5px/semibold. (c) champs de saisie : tous les `focus:border-ink` remplacés par `focus:border-active focus:ring-4 focus:ring-active/12` (11 fichiers, constantes inputClass + inline ; le titre NoteEditor garde son focus:outline). (d) Home hero harmonisé : ombre teintée bleue (décolle du fond blanc) + boutons avec micro-interaction tactile (ease-ios, -translate-y-px, active:scale-[0.97], shadow-card).
- ✅ DARK MODE (handoff dédié design_handoff_creative_space_dark/, 01/07). Stratégie = variables CSS + `darkMode:"class"`.
  - **P.R.1 fondations** : tailwind.config `darkMode:"class"` + tokens convertis en `rgb(var(--x)/<alpha-value>)` (CANAUX RGB espacés, pour garder les modificateurs d'opacité type ring-active/12) ; nouveaux tokens `bg`/`surface`/`surface-2`/`hairline`/`hairline-strong`. Variables :root (clair, valeurs IDENTIQUES aux anciens hex) + .dark (nuit) dans globals.css + utilitaires `.cs-card-surface`/`.cs-glass`. Persistance : table clé-valeur `profile` clé **"theme"** (PAS de migration). `lib/theme.ts` getTheme() ; `app/theme-actions.ts` setTheme(). Root layout `app/layout.tsx` async + `export const dynamic="force-dynamic"` → pose la classe `dark` sur <html> par requête (pas de flash). `ThemeToggle` (components/app-shell/ThemeToggle.tsx) dans le Header. Barres verre (Sidebar/Header/BottomNav) + Button en dark. **Astuce clé** : `bg-ink text-bg` (au lieu de text-white) → s'inverse correctement (pill sombre en clair, pill claire en nuit).
  - **P.R.2 surfaces de contenu** (7 sous-agents parallèles, fichiers disjoints) : variantes `dark:` ajoutées PARTOUT sans toucher le light (bg-white→dark:bg-surface, border-gray/black→border-hairline, fonds/hover neutres→white/[0.06], divides→white/10, badges statut→dark:bg-<token>/15, notes post-it filigrane dark, semainier board/tuiles/chips sombres, mini-graphe CA en currentColor donc theme-aware). RÈGLE tenue : le mode clair est strictement inchangé (uniquement ajout de `dark:`).
  - ⏳ RAFFINEMENTS restants (mineurs, à faire après validation Adrien) : (1) Diagrammes Recharts (DiagrammesSection) — tooltips/axes/ticks en hex clair en dur (#9CA3AF, #f3f4f6...), pas encore theme-aware. (2) Notes post-it : le filigrane de priorité est neutre en dark (perd la teinte couleur ; le README voulait un filigrane teinté rgba(couleur,0.14)). (3) RichText : le bouton "couleur par défaut" applique #1A1A1A en dur (peu lisible en dark si utilisé) ; le texte tapé sans couleur hérite bien de la couleur du conteneur (theme-aware). (4) viewport themeColor mobile reste #ffffff. (5) NETTOYAGE : le README dark demande de SUPPRIMER les 2 dossiers design_handoff_creative_space* une fois validé — NE PAS supprimer avant l'aval d'Adrien.

**REFONTE APP COMPLÈTE (Claude Design → maquette design_handoff_app/, en cours 01-02/07)** : Adrien a fourni une MAQUETTE d'app entière (design_handoff_app/, gitignoré : README + "Creative Space - App.dc.html" + "Système de design.dc.html" + Screenshot exemple/). Objectif = reproduire EXACTEMENT la maquette (tailles, espacements, poids typo) en gardant la logique. Consigne clé d'Adrien : "prends ce qu'il a fait EXACTEMENT, tu gardes les liens/interactions mais l'apparence pareille". Il itère par gros lots de retours, exigeant sur la fidélité (m'a repris plusieurs fois). Fait :
- **Shell** : nav pill glass HORIZONTALE centrée en haut (Accueil · Work · **Bank**(=/finance) · Freelance · **To do**(=/notes)), mots seuls + traits fins, actif = pill encre `bg-ink text-bg`. Onglet **Moi SUPPRIMÉ** (profil pro remonté en haut de /freelance). FAB "+ Note" bas-droite. Mobile = BottomNav glass flottante. Canvas teinté #F6F7FB + halos radiaux (0.09/0.075/0.06) + dérive lente. **DARK MODE ABANDONNÉ** (retiré toggle/lib/theme ; darkMode:"class" gardé pour neutraliser les variantes dark: résiduelles ; elles sont retirées écran par écran).
- **Typo** (globale) : titres section 26px/extrabold, H1 page 30px/extrabold, sous-sections 22px/bold, tracking serré. Tokens en variables CSS (rgb(var(--x)/<alpha>)).
- **Accueil** : en-tête "Bonjour Adrien" ; **À traiter** en HAUT à gauche (texte simple, dot+texte+lien coloré, données réelles + item "Déclaration URSSAF dans X j") ; **4 KPI** (Projets actifs+nom, CA du mois + dernière URSSAF déclarée, Tâches du jour x/y + barre, Prochaine échéance + nom projet) ; TodayTasks (heure DEVANT format 16h, tri par catégorie puis heure, tag catégorie + pill client) ; Cette semaine ; **bento Objectifs** (Encaissé net juin2026→juin2027 vs 16 800€=1400x12 ; compteurs abonnés Instagram+Behance ÉDITABLES via profile clés me_ig_followers/me_be_followers ; Inspiration dégradé animé + marques ; Trouver des clients).
- **Work** : bannière dégradée ; cartes projet (dot+nom qui wrap+badge, client·catégorie colorée, tags mission, barre+%) ; calendrier (board `bg-[#FCFCFD]`, boîtes catégorie TEINTÉES sans dot, tuiles `#F1F1F4`, chips 1 dot, board DÉBORDE en largeur `w-[min(1360px,94vw)]` mais titre+contrôles restent centrés, vue Semaine/**Liste**/Mois) ; clients monogramme.
- **Bank/Finance** : page titrée "Freelance" ; dashboard + mini-graphe ; URSSAF cartes-mois (mois courant = BLANC + contour bleu ring ; déclaré vert ; flèches encadrées) ; Seuils (plafond micro-BNC + objectif mensuel freelance) ; Impôt épuré ; "Salaire & impôt" barre accent violette.
- **Freelance** : profil pro en haut (monogramme AP, infos éditables flat crayon-au-survol, liens pro pastille couleur) ; guide en CARTES/BOX ; **Communication = stepper 7 étapes + carte détail unique** (scripts intégrés) ; section Brief renommée **"Questionnaire"** ; "Prospection" renommée **"Trouver des clients"** (+ liens). Arnaqueurs retiré.
- **To do** : post-its colorés (+ Post-it = vrai compositeur texte→note libre) + checklist À faire + Terminées ; contenu recentré max-w-4xl. **RichText** (gras/italique/couleur) : fix = exec sur onMouseDown (garde la sélection).
- **Catégorie ÉCOLE ajoutée** (3e : freelance/entreprise/**ecole**/perso), couleur violet #9333EA. Enum SQL → **migration 014** (ALTER TYPE calendar_category ADD VALUE 'ecole' BEFORE 'perso') à exécuter.
- **Projets non-freelance** (entreprise/école/perso) : PAS de provenance/argent/devis/facture (masqués si catégorie≠freelance dans ProjectCreateForm + ProjectOverlayBody, forcés null à la save) ; Bank ne voit QUE le freelance (toValidate/inProgress/alerte solde filtrent category==="freelance").
- **Migrations SQL : toutes push jusqu'à la 019.** Détail des dernières : 015 projects.org, **016 notes.emoji**, **017 notes.color + notes.is_task (+ backfill is_task=true pour les notes titrées)**, **018 projects.pinned**, **019 notes.is_bloc**. Adrien les lance manuellement dans Supabase (lui fournir le SQL). Si un ajout de note/tâche/bloc échoue → une migration manque.
- ⏳ RESTE (demandé, à faire) : (b) entreprise/école : "client" = SELECT figé qui NE crée PAS de fiche client → champ `projects.org` (migration 015, faite). (d) Nettoyage : supprimer design_handoff_app/ après validation finale. (e) éventuel champ "thème" pour perso (à confirmer). (f) **VERSION MOBILE à refaire (voir section dédiée dans roadmap.md "PHASE 8 — Mobile")** : c'est le gros chantier en cours.

**⚠️ MOBILE = CHANTIER EN COURS (03/07)** : plan + checklist détaillés dans **roadmap.md → PHASE 8 — Mobile**. **Passes 1 à 5 faites** : BottomNav safe-area iOS ; overlays + panneau Notion (side) en **bottom-sheet < md** (45% à droite ≥ md) ; tableau "À faire" → **cartes empilées** sur mobile ; titres réduits ; post-its (vedette pleine largeur) ; **Projets = 1 carte/écran sur mobile** (matchMedia) + flèches ramenées dans l'écran ; **"Cette semaine" retirée de l'accueil** ; **"Note rapide"** = formulaire bloc note direct (components/home/QuickNote) ; **URSSAF** : "12 derniers mois" retiré + vue Mois en 1 carte + flèches sur mobile ; **croix des overlays alignée au titre** ; **drag du semainier via une POIGNÉE (grip)** par bloc (fix : le scroll marche partout au doigt, plus de drag accidentel) ; NotePanel/BlocEditor n'auto-focus plus le titre. **Méthode : corriger le `base` (mobile) sans toucher aux variantes `md:`.** Reste : test fin ~390px, ergonomie calendrier, Freelance. **Bug ouvert : emoji des notes non appliqué sur le Brave d'Adrien (vérifié OK via test navigateur Playwright).**

**SESSION 02-03/07 (gros peaufinage desktop) — FAIT :**
- **Nav → 4 éléments** (Accueil/Work/Bank/Freelance) : onglet "To do" retiré (components/nav-items.ts) ; les notes vivent dans Work (section To do sous le calendrier). Le **FAB flottant "+ Note" est SUPPRIMÉ** (app/(main)/layout.tsx, QuickNoteButton supprimé) → création via les boutons "+ Post-it" / "+ Tâche" / icône bulle "Bloc notes" dans Work.
- **Work / Projets** : cards à **contour de la couleur** du projet, **statut en contour + texte coloré** (pas de pastille) à droite des types de mission ; **bouton "Clients"** ouvre un grand overlay (recherche + fiches détaillées non cliquables + crayon + "+") ; **pagination 3 projets + flèches** qui débordent à gauche/droite (cards pleine largeur) ; **épingle = emoji 📌** qui dépasse du coin ; tri **épinglés puis échéance la plus proche** (sans date en dernier) ; **échéance à droite** sur la ligne client·catégorie. (migration projects.pinned = 018)
- **Work / Calendrier** : "+" **en bas à droite** des cases ; vue **Mois supprimée** ; vue **Liste = carrousel** (hier grisé · jour centré · demain) navigation jour par jour ; vue **Semaine** avec boîtes de catégorie remplies + colonne gauche réduite ; **TOUTES les notes de calendrier s'ouvrent en panneau latéral façon Notion** (NotePanel `side` + `alwaysEdit`, 45% à droite, fermeture au clic à gauche, texte riche : gras/italique/listes/tailles/couleurs).
- **Work / To do** (NotesClient, sous le calendrier) : **3 types de notes** distingués par colonnes DB. (1) **Post-it** (`is_task=false, is_bloc=false`) : titre grand **coloriable** (RichText compact), corps, thème, date, emoji "épingle", **couleur choisie** ; triés par échéance, le plus proche mis en avant (plus grand + lueur). (2) **Tâches "À faire"** (`is_task=true`) : **tableau** Importance/Idée/Date/Thème, ligne **teintée à J-7** selon l'importance, éditeur **inline sans crayon** (tout modifiable au clic, autosave). (3) **Bloc notes** (`is_bloc=true`) : cartes plus hautes que larges, 2/ligne, titre + texte riche (bouton icône bulle). Thèmes = **5 fixes** (Activités, Tâches, Design, Loisir, Santé). Emoji = palette + champ libre non contrôlé.
- **Accueil** : "À traiter" = **1 rectangle compact** (rouge si urgence), **trié par échéance réelle**, préfixe "J-X :" / "Aujourd'hui" / "En retard 🚨", **popup de retard** à l'arrivée avec bouton **"Attendre 48h"** (snooze localStorage). KPI = CA du mois, Tâches du jour, **Projet à finir** (nom en avant), **Information à venir** (widget ENTIÈREMENT éditable : titre/texte/emoji/date/thème/couleur, clés profile info_*). Objectifs = **Bénéfice net · année** (net − dépenses − URSSAF), widgets **Instagram** (abonnés + progression + dernier post) et **Behance** (abonnés + appréciations) séparés, Inspiration. Fond dégradé **animé** (calque fixe, translate+scale+rotation).
- **Bank/Finance** : URSSAF calé sur le **relevé réel (117€/898€ = 13,03% ACRE, 26,06% plein)**, libellé calculé. Diagrammes : **Malt en rouge de marque**, "Identité visuelle" en **dégradé bleu**, "Argent par mois" = **facturé (orange) + net gagné (vert superposé, effet 3D)**. **Salarié : regroupé par employeur**, blocs **repliables**, le plus récent (Poppins, puis The Source) **ouvert par défaut**.
- **Freelance** : PP = **logo pztdesign** (étoile bleue #3704F0 sur fond blanc + contour bleu, components/ui/Logo.tsx + app/icon.svg = favicon). "Liens pro" supprimé + profil compact. **"Trouver des clients"** sous le profil. Communication = tunnel sans étape sélectionnée par défaut. Sections **Questionnaire et Devis retirées de la page** (déjà dans le tunnel client ; sous-routes /freelance/brief et /freelance/devis conservées).
- **Overlays** : scrim **opaque net (bg-black/55) sans backdrop-blur** (l'ancien flou créait un artefact de couleur avec la nav en verre, surtout sur Brave).
- **REPORTÉ (décision Adrien 02/07)** : objectif **Instagram/Behance auto** (API Behance fermée par Adobe ; Instagram = app Meta + token 60j) → saisie manuelle.
- ⚠️ **BUG NON RÉSOLU** : Adrien signale que l'**emoji des notes/tâches ne s'applique pas chez lui (Brave)** ; vérifié FONCTIONNEL de mon côté via test navigateur automatisé (Playwright/Chromium : clic palette + saisie champ libre → emoji à côté du titre + dans le tableau + persisté en base). Probablement spécifique à son navigateur/manip. À reproduire avec son cas exact (clic palette vs clavier emoji Mac).

**REFONTE DESIGN via Claude Design (prévue)** : Adrien veut passer par Claude Design pour construire un vrai système de design/CSS (l'app est fonctionnelle mais le design reste "basique"). Brief complet et transmissible = **docs/design-brief.md** (stack, tokens, inventaire de toutes les pages/blocs, points faibles, livrables attendus). Objectif : iOS/Apple minimal, glass 2026, Notes façon post-it, boîtes de catégorie calendrier plus élégantes, NotePanel façon Notion, animations subtiles sans lag. Le CSS proposé sera ré-implémenté en Tailwind.

**Correctifs session Notes/calendrier (01/07)** : Notes = enregistrement EXPLICITE (plus de note créée si on annule ; bouton Créer/Enregistrer ; "marquer faite" retiré de l'éditeur), poubelle par carte, couleurs priorité distinctes (rouge/ambre/gris) + accent gauche épais (plus de tag), tri échéance la plus proche en haut, éditeur partagé components/notes/NoteEditor (titre 3xl), bouton flottant "+" = même éditeur complet. Calendrier : NotePanel keyé par bloc (fix format qui bavait) + aperçu live gras/italique/couleur sur le titre. RESTE (design) : boîtes catégorie calendrier "gamines", esthétique post-it des notes, layout Notion du NotePanel, format texte par sélection (actuellement mise en forme du bloc entier).

**Retours session 01/07 (suite 3)** : (1) TEXTE RICHE remis et FONCTIONNEL : components/notes/RichText.tsx (contentEditable + execCommand bold/italic/foreColor par SÉLECTION, en direct) utilisé pour le CONTENU des notes (stocké en HTML). Aperçus/cartes/suggestions calendrier passent par stripHtml (lib/notes.ts). (2) Notes = ouverture en LECTURE + crayon Modifier (fait). (3) Calendrier : libellés catégorie = texte coloré SANS pastille + padding ; flèche week-end déplacée en petit bouton dans l'en-tête (desktop) au lieu de la colonne de droite (gain de place). (4) Adrien valide la page Notes comme référence -> RESTE À FAIRE : appliquer le MÊME format (lecture + crayon + RichText) aux pages de livrables / blocs calendrier (NotePanel), qui s'ouvrent encore en édition directe.

**Retours session 01/07 (suite 2)** : (1) Diagrammes : "Argent par provenance" et "Argent par type de mission" = DEPUIS LE DÉBUT (toutes années) ; "Argent par mois" = par année avec navigation (flèches) + vue liste affiche "Mois année". (2) Boîtes catégorie calendrier : revenues au simple (case gris clair comme les jours + texte coloré + petit point) - les pastilles à dégradé débordaient/étaient moches. (3) Format texte riche du calendrier (gras/italique/couleur par sélection) RETIRÉ : jamais fonctionnel proprement (Adrien voulait du WYSIWYG par sélection, on avait du "bloc entier" bugué). Les colonnes bold/italic/text_color restent en base (rendu conservé si déjà posé) mais plus de contrôles. **À REFAIRE proprement plus tard** = vrai éditeur de texte (contentEditable, sélection) — c'est de la LOGIQUE, pas juste du CSS, donc pas pour Claude Design. (4) ENCORE EN CHANTIER (design + logique) : ouvrir une note/livrable = **page façon Notion en lecture** (grand titre, propriétés en lignes lisibles, beaucoup d'air, % même hors projet), l'édition seulement au clic sur le crayon. NotePanel actuel = titre 3xl + propriétés en lignes mais reste un panneau latéral d'édition. À finaliser avec Claude Design (visuel) + reprise logique (mode lecture/édition).
...
**Migrations SQL (supabase/migrations/) :** schema.sql + 001 tags clients + 002 projet category/color + 003 calendar notes + 004 cancelled/mission_types/cost + 005 source/gross/net/paid/progress + 006 mission_expenses + 007 bucket banners + 008 payments.mission_type text + 009 calendar time + 010 calendar bold/italic/text_color + 011 notes title/done/priority/theme/due_date + 012 notes.deleted_at + 013 prospects + 014 catégorie école + 015 projects.org + **016 notes.emoji + 017 notes.color & is_task (backfill) + 018 projects.pinned + 019 notes.is_bloc**. Adrien les exécute manuellement dans Supabase (SQL Editor) au fur et à mesure ; lui fournir le SQL. **Toutes push jusqu'à 019.**

**PAGE NOTES refondue (01/07/2026)** : vraie to-do. Table notes enrichie (migration 011). NotesClient : liste priorisée (haute/moyenne/basse, tri prio->échéance->récence), carte avec case à cocher + accent couleur de priorité + thème + échéance (rouge si dépassée), éditeur en overlay (titre, détails, priorité, thème, échéance, faite, supprimer), section "Terminées" repliable, états motivants ("Tout est fait 🎉"). QuickNoteButton inchangé (createNote(content) reste compatible). **Note -> calendrier** : les notes non faites apparaissent dans le "+" de la ligne PERSO du calendrier ("Notes à planifier") ; les choisir crée un bloc Perso. CalendarSection reçoit `notes` depuis la page Work. Toujours lui fournir le SQL et lui dire de l'exécuter (il est débutant, le faire pas à pas).
...
- **DESIGN SYSTEM (01/07/2026)** : FOND BLANC (le canvas gris iOS a été testé puis REJETÉ par Adrien le 01/07 - "horrible" ; body en bg-white). On garde le reste du vernis : barres (Header/Sidebar/BottomNav) = verre dépoli `bg-white/70 backdrop-blur-xl backdrop-saturate-150 border-black/[0.06]` ; overlays = feuille `rounded-3xl`, fond `bg-black/30 backdrop-blur-sm`, poignée grise sur mobile ; Home et Moi ont un hero (dégradé bleu->indigo pour Home). Style épuré, arrondis généreux, hairlines légères. NB : beaucoup de cartes ont un `bg-white` explicite (résidu du test canvas gris) - inoffensif sur fond blanc. Les champs input/textarea/select ont un fond blanc forcé dans globals.css.
