...
- **Décision URSSAF** : le CA à déclarer = sommes effectivement encaissées en freelance dans le mois = `net_amount` des paiements `paid` du mois (ce qui tombe sur le compte), hors salaire, hors devis non payés, sans TVA (franchise). Confirmé par Adrien.
