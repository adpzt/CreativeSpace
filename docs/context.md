...
- ⏳ RESTE (prochain chat) : (a) mini-graphe Recharts AreaChart dans une carte stat (README 2.5 - dégradé #2563EB 0.28→0), optionnel. (b) badges de statut (README 2.7 : rounded-full px-3 py-1.5 + dot ; actuellement PAYMENT_STATUS/PROJECT_STATUS.badge dans lib/finance.ts + lib/work.ts). (c) champs de saisie (focus ring-4 ring-active/12) - inputClass dans les forms. (d) Home hero (déjà un hero dégradé, à harmoniser). (e) GROS : appliquer le format PAGE NOTION (lecture + crayon, comme les Notes) aux LIVRABLES / blocs du calendrier via NotePanel (components/ui/NotePanel.tsx) — actuellement NotePanel s'ouvre direct en édition ; il faut un mode lecture + crayon + le RichText pour le contenu. Recettes : design_handoff_creative_space/README.md.
