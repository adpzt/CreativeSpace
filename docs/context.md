...
- **Migrations à exécuter par Adrien** : 013_prospects.sql (si ajout prospect échoue), 014_categorie_ecole.sql (École). **Toutes les migrations sont push jusqu'à la 015 (confirmé 02/07).**
- ⏳ RESTE (demandé, à faire) : (b) Pour entreprise/école : le "client" = SELECT figé (Entreprise: The Source/Poppins ; École: IIM Digital School/LISAA Design Graphique) qui NE crée PAS de fiche client → champ `projects.org` (migration 015, faite). (d) Nettoyage : supprimer design_handoff_app/ après validation finale. (e) éventuel champ "thème" pour perso (à confirmer).

**⚠️ MOBILE À REFAIRE (noté 02/07)** : toute la partie mobile est peu fonctionnelle et sera **presque entièrement refaite plus tard**. Pour l'instant on peaufine À FOND la version **DESKTOP uniquement**. Ne pas investir sur le responsive mobile tant qu'Adrien ne le redemande pas.

**SESSION 02/07 (peaufinage desktop) — FAIT :**
- **Nav → 4 éléments** : onglet "To do" retiré (components/nav-items.ts). Les notes vivent désormais DANS Work (sous le calendrier) + restent accessibles par le bouton flottant "+".
- **Work / Projets — cards** : plus de pastille de couleur devant le titre ; la CARD prend un **contour de la couleur** du projet (style inline borderColor). Statut sans pastille : juste un **contour + texte de la couleur** fonctionnelle (STATUS_OUTLINE dans ProjectsSection). Statut sur sa **propre ligne au-dessus du titre** pour que le titre reste horizontal.
- **Work / Projets — Clients** : la bascule Projets/Clients est SUPPRIMÉE. La section reste "Projets" ; un **bouton "Clients"** ouvre un **overlay** avec les cards clients existantes + bouton "+". Clic sur une card → overlay client habituel.
- **Work / Calendrier** : (1) le "+" de chaque case est **en bas à droite** (Cell = flex col, + dans un footer justify-end). (2) Vue **Mois SUPPRIMÉE** (MonthView retiré, view = "week"|"list"). (3) Vue **Liste = carrousel** : hier (grisé, gauche, aperçu) · jour au centre (carte interactive) · demain (droite, aperçu) ; flèches du haut naviguent **jour par jour**. (4) Vue **Semaine** : colonne de gauche réduite (132→104px) et boîtes de catégorie **remplissent la colonne** (rounded-2xl teinté, cf. maquette) pour gagner le vide blanc à gauche.
- **Work** : sous le calendrier, ajout de **"To do"** (NotesClient) puis **"Trouver des clients"** (ProspectsBoard, déplacé depuis Freelance).
- **Accueil** : "À traiter" **trié par priorité** (urgent > à surveiller > info). **TodayTasks** resynchronise son état sur les props serveur (useEffect) → les MAJ de titre/heure/notes faites ailleurs remontent sans refresh. Fond dégradé **plus vivant** (halos qui bougent + respirent, globals.css). Bento "Trouver des clients" pointe vers /work.
- **Freelance** : PP = **logo pztdesign** (étoile blanche sur dégradé, components/ui/Logo.tsx). Section "Liens pro" **supprimée** + profil **plus compact**. Communication : **aucune étape enfoncée à l'arrivée** (active=null, invite affichée). "Brief" renommé **"Questionnaire"** partout (stepper, tunnel data). Devis : les **checklists redondantes retirées** (garde pénalités + liens + CGP) — à confirmer par Adrien que c'était bien "les info en bas" visées.
- **REPORTÉ (décision Adrien 02/07)** : objectif **Instagram/Behance auto**. L'API Behance publique est **fermée par Adobe (2020)** → auto impossible ; Instagram nécessite une **app Meta + token à renouveler tous les 60j** créés par Adrien. Reste en **saisie manuelle** (FollowerCounter). À reprendre plus tard si Adrien monte l'app Meta.
...
**Migrations SQL exécutées par Adrien (dans Supabase) :** schema.sql initial + 001 (tags clients) + 002 (projet category/color, deliverable notes) + 003 (calendar_blocks.notes) + 004 (statut cancelled, mission_types, cost) + 005 (source, gross/net, paid, deliverable progress) + 006 (mission_expenses jsonb) + 007 (bucket storage banners). **À EXÉCUTER par Adrien : 008 (payments.mission_type enum -> text), 009 (calendar_blocks.time), 010 (calendar_blocks bold/italic/text_color), 011 (notes title/done/priority/theme/due_date), 012 (notes.deleted_at = corbeille).**
