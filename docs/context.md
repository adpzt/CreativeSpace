...
- ⏳ RESTE (prochain chat) : (a) mini-graphe Recharts AreaChart dans une carte stat (README 2.5 - dégradé #2563EB 0.28→0), optionnel. (b) badges de statut (README 2.7 : rounded-full px-3 py-1.5 + dot ; actuellement PAYMENT_STATUS/PROJECT_STATUS.badge dans lib/finance.ts + lib/work.ts). (c) champs de saisie (focus ring-4 ring-active/12) - inputClass dans les forms. (d) Home hero (déjà un hero dégradé, à harmoniser).
