...
- **Migrations à exécuter par Adrien** : 013_prospects.sql (si ajout prospect échoue), 014_categorie_ecole.sql (École).
- ⏳ RESTE (demandé, à faire) : (a) Work : mettre Clients dans un **bouton discret DANS la section Projet** (toggle Projets/Clients). (b) Pour entreprise/école : le "client" = SELECT figé (Entreprise: The Source/Poppins ; École: IIM Digital School/LISAA Design Graphique) qui NE crée PAS de fiche client → prévoir champ `projects.org` (migration 015) ou équivalent. (c) **Responsive MOBILE** à peaufiner partout. (d) Nettoyage : supprimer design_handoff_app/ après validation finale. (e) éventuel champ "thème" pour perso (à confirmer).
