...
**PHASE 4 (Freelance) EN COURS** : hub /freelance (5 cartes) FAIT. 4.1 Communication client FAIT (tunnel 7 étapes dépliables, 13 scripts copiables avec ancres, red flags ; contenu dans lib/freelance.ts, vue dans components/freelance/CommunicationView.tsx + CopyButton). Routes brief/devis/production/prospection = placeholders avec lien retour. RESTE : 4.2 Brief, 4.3 Devis & CGP, 4.4 Production, 4.5 Prospection (board prospects = la seule partie avec DB, table `prospects` déjà au schéma).

**À faire ensuite :** finir Phase 4, Phase 5 (Home/dashboard alertes), Phase 6 (Moi), Phase 7 (bonus : PWA, dark mode, etc.).
