...
- 🔜 PROCHAINE ÉTAPE = DARK MODE (Adrien la demandera juste après, il donnera ses consignes). Préparer : le CSS a déjà un commentaire "theme clair uniquement, dark plus tard" (globals.css) et beaucoup de couleurs sont en tokens (ink, ink-soft, muted, active...) + hex en dur (fonds blancs explicites, dégradés). Prévoir une stratégie (variables CSS ou dark: Tailwind + tokens dérivés). Ne PAS commencer sans ses consignes.
